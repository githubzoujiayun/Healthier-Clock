...
public class JSONHelper {
  ...
  public static String toJSON(Object obj) {
    ...
    serialize(json, obj);
    return json.toString(); // {"username":"dfasdf", "email":"email@gmail.com"
                            // ... }
  }

  /** 序列化为JSON **/
  private static void serialize(JSONStringer json, Object o) {
    if (isNull(o)) {
      try {
        json.value(null);
    ...
    if (isObject(clazz)) { // 对象
      serializeObject(json, o);
    } else if (isArray(clazz)) { // 数组
      serializeArray(json, o);
    } else if (isCollection(clazz)) { // 集合
      Collection<?> collection = (Collection<?>) o;
      serializeCollect(json, collection);
    } else { // 单个值
      try {
        json.value(o);
      } catch (JSONException e) {
   ...
   * @throws
   **/
  public static <T> T parseObject(JSONObject jsonObuect, Class<T> clazz) {
    if (clazz == null || isNull(jsonObuect)) {
      return null;
    }
    ...
    for (Field f : clazz.getFields()) {
      setField(instance, f, jsonObuect);
    }
  ...
  /** 设定字段的值 **/
  private static void setField(Object instance, Field f, JSONObject jo) {
